...
(function ($) {
    ...
    let HHActions = {
        ...
        initCheckboxAction: function (el) {
            var base = this;
            $('body').on('change click', '.hh-checkbox-action', function (ev) {
                var t = $(this),
                    parent = t.closest(t.data('parent'));
                ev.preventDefault();
                var data = JSON.parse(Base64.decode(t.attr('data-params')));
                if (typeof data == 'object') {
                     if ( t.is('[data-value]') ) {
                        data['val'] =parseInt(t.data('value'))  ;
                      }
                      else{
...
var translations = {
    stageNumber: window.vote.stageNumber || 'My default translation',
    Excluded: window.vote.Excluded || 'My default translation',
      StageStartDate: window.vote.StageStartDate || 'My default translation',
        StageEndDate: window.vote.StageEndDate || 'My default translation',
};

$("#add_stage").click(function(e) {

    let html = ` <div class="row" style="position: relative;">
                              <div style="    position: absolute;top: 40px;right: -8px;">
                                ...
                                <div class="col-md-3">
                                    <label for="stage number">`+translations.stageNumber+`
                                    </label>
                                   
                                    <input type="text" name="name[]" class="form-control">
                                </div>
                                   <div class="col-md-3">
                                    <label for="stage number">`+translations.Excluded+`
                                    </label>
                                    <input type="text" name="per[]" class="form-control">
                                </div>
                                
                                <div class="col-md-3">
                                    <label for="Stage start date">`+translations.StageStartDate+`</label>
                                    <input type="date" class="form-control" name="start_date[]" id="">
                                </div>
                                <div class="col-md-3">
                                    <label for="Stage end date">`+translations.StageEndDate+`
                                    </label>
...
// $(document).find('#experienceID').val(exID);
// });
  ...
  $('.addSub').on("click", function () {
    ...
    if(id != undefined){
      ...
      var experienceID=$(this).data('experience_id');
    $(document).find('#experienceID').val(experienceID);
    $.ajaxSetup({
        headers: {
     ...
     $.ajax({
         ...
         processData: false,
         success: (data) => {
             $(document).find('#id').val(data[0].id);
             $(document).find('#link').val(data[0].link);
         $(document).find('#name').val(data[0].name);
         $(document).find('#age').val(data[0].age);
         $(document).find('#type').val(data[0].type);
         $(document).find('#blah').attr("src",data[1]);
        //  $(document).find('#file').val(data[1]);
            $("#vote").modal()
         },
         error: function (data) {
             console.log(data);
