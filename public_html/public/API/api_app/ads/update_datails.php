...
if($auth_key == "aSdFgHjKl12345678dfe34asAFS%^sfsdfcxjhASFCX90QwErT@") {
    ...
    $sql20 = "INSERT INTO `smartend_booleanFeatureAqar` (`id_BF_aqar`, `state`, `id_description`, `id_BFAT`) VALUES (NULL, '".$isYard."', '".$id_description."', '17');";
	...
	UPDATE `smartend_adsDescription` SET `space`='".$totalSpace."',`price`='".$totalPrice."',`description`='".$detailsAqar."',`id_typeRes`='".$selectedPlan."',`id_type_aqar`='".$selectedTypeAqar."',`id_interface`='".$interfaceSelected."',`id_adder_relation`='".$selectedAdderRelation."',`id_marketer_relation`='".$selectedMarketerRelation."' WHERE `smartend_adsDescription`.`id` = '".$id_description."'
	
	";
	
	
	if (mysqli_query($mysqli, $sql31)) {
    } else {
      //echo "Error: " . $sql1 . "<br>" . mysqli_error($mysqli);
    }
	
	/*.........end update adsDescription...................*/
	...
	UPDATE `smartend_ads` SET `title`='".$title."',`timeUpdated`=CURRENT_TIMESTAMP,`id_category`='".$id_category."' WHERE `ads`.`id_description`='".$id_description."'
