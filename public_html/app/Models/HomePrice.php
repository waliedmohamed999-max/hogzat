...
class HomePrice extends Model
{
    ...
    public function getPriceItems($home_id, $start, $end, $status = 'all')
    {
     ...
     public function getLastMinute( $tomorrow, $status = 'all' ,$homeID)
    {
        $tomorrow = esc_sql($tomorrow);
      
        $sql = DB::table($this->getTable())->whereRaw("(home_price.start_time = {$tomorrow}  )");
        if ($status != 'all') {
            $sql->where('available', $status);
        }

        if ($status != 'all') {
            $sql->where('home_id', $homeID);
        }
