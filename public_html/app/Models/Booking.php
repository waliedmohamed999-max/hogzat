...
class Booking extends Model
{
    ...
    {
        $sql = DB::table($this->getTable())->selectRaw("sum(total) as total")->where('status', 'completed');
        if (!empty($userID)) {
            if ($userType == 'owner') {
                $sql->where('owner', $userID);
            } else {
...
public function getNetAmountP($userID = '', $userType = 'owner')
    {
        $sql = DB::table($this->getTable())->selectRaw("sum(total) as total")->where('status', 'completed');
        if (!empty($userID)) {
            if ($userType == 'owner') {
        ...
        $sql->where('service_type', 'home');
        $result = $sql->get()->first();
        if (!empty($result) && is_object($result)) {
            return (($result->total)/1.05);
        }
        return 0;
    }
      public function getNetAmountE($userID = '', $userType = 'owner')
    {
        $sql = DB::table($this->getTable())->selectRaw("sum(total) as total")->where('status', 'completed');
        if (!empty($userID)) {
            if ($userType == 'owner') {
        ...
        $sql->where('service_type', 'experience');
        $result = $sql->get()->first();
        if (!empty($result) && is_object($result)) {
            return (($result->total)/1.05);
        }
