...
class MenuStructure extends Model
{
    ...
    {
        $query = DB::table($this->table)->where('menu_id', $menu_id);
        if (DB::getSchemaBuilder()->hasColumn($this->table, 'menu_lang')) {
            $query->where('menu_lang', get_current_language());
        }
        if (DB::getSchemaBuilder()->hasColumn($this->table, 'is_active')) {
            $query->where('is_active', 1);
        }
        if (DB::getSchemaBuilder()->hasColumn($this->table, 'sort_order')) {
            $query->orderBy('sort_order');
        } else {
            $query->orderBy('id');
        }
        $menus = $query->get();
        if ($menus->isEmpty() && DB::getSchemaBuilder()->hasColumn($this->table, 'menu_lang')) {
            $fallback = DB::table($this->table)->where('menu_id', $menu_id);
            if (DB::getSchemaBuilder()->hasColumn($this->table, 'is_active')) {
                $fallback->where('is_active', 1);
            }
            if (DB::getSchemaBuilder()->hasColumn($this->table, 'sort_order')) {
                $fallback->orderBy('sort_order');
            } else {
                $fallback->orderBy('id');
            }
            $menus = $fallback->get();
    ...
    public function getAdminByMenuId($menu_id)
    {
        $query = DB::table($this->table)->where('menu_id', $menu_id);
        if (DB::getSchemaBuilder()->hasColumn($this->table, 'menu_lang')) {
            $query->where('menu_lang', get_current_language());
