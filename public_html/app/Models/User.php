...
class User extends Authenticatable
{
    ...
    public function allPartnerRequest($data)
    {
        ...
        $query = DB::table($this->getTable())->selectRaw('SQL_CALC_FOUND_ROWS users.*, roles.slug as role_slug, roles.name as role_name, agreement.status as status')->join('role_users', 'users.id', '=', 'role_users.user_id', 'inner')
            ->join('roles', 'role_users.role_id', '=', 'roles.id', 'inner')->join('agreement', 'users.id', '=', 'agreement.user_id', 'inner');
        $number = $data['number'];
        if (!empty($number)) {
        ...
        $query->whereIn('roles.id', [3, 2]);

        $query->whereIn('users.request', ['request_a_partner', 'request_a_customer']);

        if (!empty($data['rol'])) {
                  $query = DB::table($this->getTable())->selectRaw
                  ('SQL_CALC_FOUND_ROWS users.*, roles.slug as role_slug, roles.name as role_name , agreement.status as status ')
                  ->join('role_users', 'users.id', '=', 'role_users.user_id', 'inner')
            ->join('roles', 'role_users.role_id', '=', 'roles.id', 'inner')->join('agreement', 'users.id', '=', 'agreement.user_id', 'inner');
            $query->whereIn('users.request', ['request_a_partner', 'request_a_customer']);
            $query->whereIn('roles.id',  [3, 2]);
    ...
    {
        return DB::table($this->getTable())->where('mobile', $mobile)->first();
    }
