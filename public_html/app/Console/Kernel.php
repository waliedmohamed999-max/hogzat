...
class Kernel extends ConsoleKernel
{
     ...
     */
    protected $commands = [
          Commands\VoteCron::class,
        Commands\LastMinuteCron::class,
    ...
    {
         $schedule->command('VoteCron:cron')->daily();
         $schedule->command('LastMinuteCron:cron')->everyMinute();
        $this->payout($schedule);
        $this->ical($schedule);
         $schedule->command('deleteRevisionCron:cron')->daily();
    }
    ...
    private function payout($schedule)
    {
        ...
        $schedule->command('payout:calculate')->cron('0 0 */3 * *');
    }
