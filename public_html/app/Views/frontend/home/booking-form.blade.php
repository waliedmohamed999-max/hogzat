...
<form class="form-action" action="{{ url('add-to-cart-home') }}" method="post" data-loading-from=".form-body"
    ...
    @if($post->booking_type == 'per_night')
        <div class="form-group" style="{{ isset($post->last) ? 'display: none' : ''}}">
            ...
            <div class="date-wrapper date-double"
                ...
                <input type="text" class="input-hidden {{ isset($post->last) ? ''  : 'check-in-field' }}" value="{{ isset($post->last) ?  date("Y-m-d",$post->start_time_last)  : '' }}"
                       name="checkIn">
                    
                <input type="text" class="input-hidden {{ isset($post->last) ? ''  : 'check-out-field' }}" value="{{ isset($post->last) ?  date("Y-m-d",strtotime('+1 day',$post->start_time_last))  : ''}}"
                       name="checkOut">
                <span class="check-in-render"
                      data-date-format="{{ isset($post->last) ?  date(hh_date_format(),$post->start_time_last)  : hh_date_format_moment() }}"></span>
        ...
        <div class="form-group"  style="{{ isset($post->last) ? 'display: none' : ''}}">
            ...
            <div class="date-wrapper date-single"
                ...
                <input type="text"
                       ...
                       name="checkInOut" data-home-id="{{ $post->post_id }}"
                       data-home-encrypt="{{ hh_encrypt($post->post_id) }}">
                <input type="text" class="input-hidden {{ isset($post->last) ? ''  : 'check-in-field' }}" value="{{ isset($post->last) ?  date("Y-m-d",$post->start_time_last)  : hh_date_format_moment() }}"
                       name="checkIn">
                <input type="text" class="input-hidden {{ isset($post->last) ? ''  : 'check-out-field' }}" value="{{ isset($post->last) ?  date("Y-m-d",strtotime('+1 day',$post->start_time_last))  : hh_date_format_moment() }}"
                       name="checkOut">
                <span class="check-in-render"
                      data-date-format="{{ isset($post->last) ?  date("Y-m-d",$post->start_time_last)  : hh_date_format_moment() }}"></span>
            </div>
        </div>
    ...
    <div class="form-group row">
        <div class="col-md-12">
             <label >{{__('Entry and exit time')}}</label>
        </div>
       
   
           @if($checkIn)
                            <div class="col-md-6 item d-inline-block  mb-3">
                                <span class="font-weight-bold"><i class=" ti-time mr-1"></i>{{__('Check-in Time:')}}</span>
                                <span class="ml-2">
                                    @if(strpos($checkIn, 'AM') !== false)
                                    {{ str_replace("AM",'صباحاً',$checkIn) }}
                                    @else
                                    {{ str_replace("PM",'مساءً',$checkIn) }}
                                    @endif
                                    </span>
                            </div>
                        @endif
                        @if ($checkOut)
                            <div class="col-md-6 item d-inline-block  mb-3">
                                ...
                                <span class="ml-2">
                                     @if(strpos($checkIn, 'AM') !== false)
                                    {{str_replace('AM','صباحاً',$checkOut) }}
                                     @else
                                     {{str_replace('PM','مساءً',$checkOut) }}
                                     @endif
                                    </span>
                            </div>
                        @endif
    ...
    <div class="form-group mt-2">
        ...
        <input type="hidden" name="last" value="{{ isset($post->last) ? $post->id : ''}}">
        <input type="hidden" name="homeEncrypt"
               value="{{ hh_encrypt($post->post_id) }}">
        <input type="submit" class="btn  btn-block text-uppercase booking-now"
               name="sm"
               value="{{__('Book Now')}}">
    </div>
