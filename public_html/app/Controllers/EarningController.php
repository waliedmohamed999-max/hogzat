...
class EarningController extends Controller
{
    ...
    public function updateEarning($booking_id,$type='')
    {
        ...
        $netp = $booking_model->getNetAmountP($ownerID);
        $nete = $booking_model->getNetAmountE($ownerID);
        $payout = isset($earningItem->payout) ? $earningItem->payout : 0;
        $commission = (float)get_option('partner_commission', 0);
        $ECommission = (float)get_option('partner_commission_ex', 0);
        $netp -= ($netp * ($commission) / 100);
        $nete -= ($nete * ($ECommission) / 100);
