...
class UserController extends Controller
{
    ...
    public function _afterRegisterPartner(Request $request, $return)
    {
        ...
        if (is_object($return)) {
             $user = get_user_by_id($return->user_id);
             $checkuser=checkUser($user->mobile);
            $user_id = $return->user_id;
            $timestamp = $return->timestamp;
            if (verify_time($timestamp, 5)) {
                return view('frontend.welcome-user', ['mobile' => $user->mobile]);
            }
        }
        return redirect(url('/'));
    }
    ...
    public function _becomeAHostPost(Request $request)
    {
        ...
        $validator = Validator::make($request->all(),
            ...
            [
                ...
                'mobile.required' => __('The Phone is required'),
