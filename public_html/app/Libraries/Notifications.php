...
class Notifications
{
    ...
    public function _addNotiWhenCreatedBooking($booking_id)
    {
        ...
        $this->addNotification(0, $admin->getUserId(),
            sprintf(__('A new booking <a href="%s">#%s</a>'), dashboard_url('bookings'), $booking->booking_id),
            $message,
            'booking');
        ...
        $this->addNotification(0, $customer->getUserId(),
            sprintf(__('Your booking <a href="%s">#%s</a>'), dashboard_url('bookings'), $booking->booking_id),
            $message,
            'booking');
    }
