...
export default async function ExperienceInclusionsPage() {
  const headerStore = await headers();
  const terms = await getDashboardTerms("experience-inclusions", headerStore.get("cookie") ?? "");

  return (
    <div className="space-y-6">
      <section className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-sm">
        <h1 className="text-3xl font-semibold text-slate-950">الميزات</h1>
      </section>
