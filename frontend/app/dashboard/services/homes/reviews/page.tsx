...
export default async function HomeReviewsPage() {
  ...
  const reviews = await getDashboardReviews("home", headerStore.get("cookie") ?? "");

  return (
    <div className="space-y-6">
      <section className="rounded-lg border border-slate-200 bg-white p-8 shadow-[0_20px_60px_-35px_rgba(15,23,42,0.18)]">
        <h1 className="text-3xl font-semibold text-slate-950">تعليقات الإعلانات</h1>
        <p className="mt-3 text-sm text-slate-600">مراجعة تعليقات الشاليهات والفلل من داخل لوحة التحكم الجديدة.</p>
      </section>
        ...
        {(reviews ?? []).map((review) => (
          <article key={review.id} className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <Link href={`/home/${review.service_id}/details`} className="text-sm font-medium text-rose-500">
                عرض الإعلان
              </Link>
            </div>
